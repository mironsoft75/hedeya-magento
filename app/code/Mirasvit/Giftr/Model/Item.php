    ...
    public function changeItemQty($order)
    {
        ...
        if (in_array($status, array_merge($invoicedStatuses, $receivedStatuses, $canceledStatuses))) {
            ...
            foreach ($itemsCollection as $product) {
                if ($itemId = $product->getBuyRequest()->getItemId()) {
                    /** @var \Mirasvit\Giftr\Model\Item $item */
                    $item = $this->itemFactory->create()->loadWithOptions($itemId);
                    if ($item->getId() && ($item->getProductId() === $product->getProductId())) {
                        ...
                        if (in_array($status, $invoicedStatuses)) {
                            ...
                            $item->setQtyOrdered($item->getQtyOrdered() + $product->getQtyOrdered());
                            $item->addOption([
                            ...
                            $newOrderedItems[$item->getId()] = (int) $product->getQtyOrdered();
                        }
                        ...
                        if (in_array($status, $receivedStatuses)) {
                            ...
                            $item->setQtyReceived($item->getQtyReceived() + $product->getQtyOrdered());
                            $item->addOption([
                                'product_id' => $item->getProductId(),
